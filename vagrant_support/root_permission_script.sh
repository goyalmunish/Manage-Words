...
echo "<--- Start of 'root_permission_script' Script --->"
###### Initial check ######
if [ "$EUID" -ne "0" ] ; then
...
echo User: $USER
if (which java > /dev/null) || false ; then
  echo 'Skipping Java8..'
else
...
echo User: $USER
if (which java > /dev/null) || true ; then
  echo 'Skipping Java6..'
else
...
echo "<--- Installing GIT --->"
echo User: $USER
if (which git > /dev/null) || false ; then
  echo 'Skipping GIT..'
else
  echo 'Installing GIT..'
...
echo User: $USER
if (which memcached > /dev/null) || false ; then
  echo 'Skipping Memcached Server..'
else
  echo 'Installing Memcached Server..'
...
echo User: $USER
if (which R > /dev/null) || false ; then
  echo 'Skipping R..'
else
  echo 'Installing R..'
...
echo User: $USER
if (which mysql > /dev/null) || false ; then
  echo 'Skippng Mysql..'
else
  echo 'Installing MySQL..'
...
if (which psql > /dev/null) || false ; then
  ...
  sudo su - postgres <<EOF
    ...
    psql -d template1 -c "ALTER USER postgres with encrypted password $postgres_postgres_passwd_quoted;" -a
    # TODO: Note: executing single command in postgres
EOF
...
echo User: $USER
if (which watchman > /dev/null) || false ; then
  echo 'Skipping Watchman..'
else
  echo 'Installing Watchman..'
...
echo User: $USER
if (which node > /dev/null) || false ; then
  echo 'Skipping Node..'
else
...
echo User: $USER
if (which ember > /dev/null) || false ; then
  echo 'Skipping Ember..'
else
  echo 'Installing EmberJS..'
...
echo User: $USER
if (which bower > /dev/null) || false ; then
  echo 'Skipping Bower..'
else
  echo 'Installing Bower..'
...
echo User: $USER
if (which phantomjs > /dev/null) || false ; then
  echo 'Skipping PhantomJS..'
else
  echo 'Installing PhantomJS..'
...
echo User: $USER
if (which phantomjs > /dev/null) || false ; then
  echo 'Skipping npm and bower install..'
else
  echo 'Running npm install and bower install..'
  npm install
  cd /vagrant/frontend
  npm install
  bower install
...
###### THE END ######
echo "<--- End of 'root_permission_script' Script --->"
