...
# Note that as we are referencing AppSetting class, it has to exist before running this script,
# otherwise it will create problem if you try to run "rake db:migrate" on newly created database
APP_SETTING_LOADED = true
begin
  AppSetting.first
rescue => ex
  APP_SETTING_LOADED = false
  puts "Note: AppSetting model is yet not loaded"
  puts "If unhandled, it can raise Exception: #{ex.message}"
  # class AppSetting < ActiveRecord::Base
  #   puts "Note: Creating a dummy AppSetting.get method"
  #   def self.get(str)
  #     nil
  #   end
  # end
...
if APP_SETTING_LOADED
  puts "APP_SETTING_LOADED = #{APP_SETTING_LOADED}"
  #### DEFINING CONSTANTS ###########################
  ...
  puts "Skipping setting constants defined in Initializer file #{$0}"
  puts "APP_SETTING_LOADED = #{APP_SETTING_LOADED}"
end
